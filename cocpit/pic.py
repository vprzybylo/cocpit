...
class Image():
    ...
    def resize_stretch(self, desired_size=1000):
        ...
        self.im = cv2.resize(self.image_og, (desired_size, desired_size),\
    ...
    def find_contours(self):
        ...
        self.contours, _ = cv2.findContours(self.thresh,\
                                                    cv2.RETR_EXTERNAL,\
...
#         plt.show()
    ...
    def morph_contours(self):
        ...
        self.contours, _ = cv2.findContours(image_close,\
                                            cv2.RETR_EXTERNAL,\
                                            cv2.CHAIN_APPROX_SIMPLE)
        ...
        self.contours, _ = cv2.findContours(draw,\
                                            cv2.RETR_EXTERNAL,\
                                            cv2.CHAIN_APPROX_SIMPLE)
...
#         plt.show()
    ...
    def calculate_largest_contour(self):
        self.largest_contour = sorted(self.contours,\
    ...
    def flip_imgs(self, save_dir):
        ...
        plt.imsave(save_dir+self.filename[:-4]+'_ud.png',\
                   np.flipud(self.image_og))
        plt.imsave(save_dir+self.filename[:-4]+'_lr.png',\
                   np.fliplr(self.image_og))
        plt.imsave(save_dir+self.filename[:-4]+'_ud_lr.png',\
    ...
    def save_image(self, save_dir, flip = False):
        ...
        if flip:
            ...
            cv2.imwrite(os.path.join(save_dir, \
                                     str(self.filename)), np.array(self.im))
